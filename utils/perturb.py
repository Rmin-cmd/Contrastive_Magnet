...
def direction_perturb_unsigned(pos_edges, ratio=0.1):
    n = len(pos_edges)
    idx = random.sample(list(range(n)), int(n*ratio))
    new_pos_edges = pos_edges[idx]
    survive = list(set(list(range(n))).difference(set(idx)))
...
def composite_perturb_unsigned(pos_edges, ratio=0.1):
    # pos_edges, neg_edges = sign_perturb(pos_edges, neg_edges, ratio=ratio)
    pos_edges= direction_perturb_unsigned(pos_edges, ratio=ratio)
    return pos_edges
